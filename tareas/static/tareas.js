...
function dibujarTablero() {
  ...
  [...mapaTareas.values()]
    .sort((a, b) => (a.creado_en || 0) - (b.creado_en || 0))
...
function crearTarjeta(tarea) {
  ...
  encabezado.className = "d-flex justify-content-between align-items-start";
  ...
  encabezado.append(titulo, botonEditar);
...
function crearBotonMover(etiqueta, estilo, tarea, nuevoEstado) {
  ...
  boton.addEventListener("click", () => {
    doc.transact(() => {
      mapaTareas.set(tarea.uuid, { ...tarea, estado: nuevoEstado });
    }, ORIGEN_LOCAL);
  });
  return boton;
}
